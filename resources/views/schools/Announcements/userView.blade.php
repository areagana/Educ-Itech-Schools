    ...
    <div class="container-fluid">
        <div class="h3 header">Announcements</div>
            <ul class="nav nav-tabs">
                ...
                @if(Auth::user()->hasRole(['ict-admin','school-administrator','administrator','superadministrator']))
                <li class="nav-item">
                ...
                <div class="tab-pane active" id="notices">
                    @if($notices->count() > 0)
                        @foreach($notices as $notice)
                            @if(Auth::user()->hasRole(json_decode($notice->users)))
                                <div class="border border-primary shadow-sm bg-white mt-2">
                                    <div class="header p-2 h5">
                                        {{$notice->announcement_title}}
                                            @if(Auth::user()->hasRole(['administrator','school-administrator','ict-admin','superadministrator']))
                        ...
                        <div class="form-group mt-2">
                            ...
                            @foreach($roles as $level)
                                ...
                                <div class="form-check mt-2">
                                    <input type="checkbox" class="form-check-input" name='school_level[]' id='school_level{{$level->id}}' value="{{$level->id}}">
